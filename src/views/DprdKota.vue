    ...
    <v-layout row wrap>
      <v-flex xs12>
        <v-card flat color="transparent">
          ...
          >
            <v-card-subtitle
              class="px-0 py-1 pb-0 black--text text-h6 font-weight-bold white--text"
              style="text-align: center"
            >
              - DPRD KOTA KABUPATEN -
            </v-card-subtitle>
            <v-card-subtitle
              class="px-0 py-0 black--text text-caption font-weight-bold white--text"
              style="text-align: center"
            >
              - {{ dataDapil?.dapil?.nama?.toUpperCase() }} -
            </v-card-subtitle>
            <v-divider></v-divider>
...
export default {
  ...
  computed: {
    ...mapGetters({
      dataDapil: "dapil/dapilDprdKota",
    }),
  },

  methods: {
    ...mapActions({
      getDataDapil: "dapil/getDapilDprdKota",
    ...
    async getDapil() {
      let queryDapil = {
        wilayah: this.dataWilayah.kode_wilayah.substring(0, 6),
      };
      let queryHasil = {
        uid_tps: this.dataWilayah.uid_tps,
      };
      ...
      if (this.state == "SAVE") {
        if (this.foto.length > 0) {
          ...
          this.foto.forEach((item, index) => {
            ...
            }-CAMAT-${index + 1}-${timeStamp}-DPRDKABKOTA.${item.file.name
              .split(".")
              .pop()}`;
            newFoto.push(
