      ...
      <v-flex xs12>
        <v-card flat color="transparent">
            ...
            <v-row no-gutters align="center" justify="center">
              <v-col cols="12" class="col-lg-6">
                ...
                <v-card-title
                  class="py-0 text-caption white--text font-weight-bold"
                >
                  {{ dataWilayah?.kelurahan?.toUpperCase() }} / TPS
                  {{ dataWilayah?.nomor }}
                </v-card-title>
              </v-col>
            </v-row>
          ...
          >
            ...
            >
              - DPRD PROVINSI / {{ dataDapil?.dapil?.nama?.toUpperCase() }} -
            </v-card-subtitle>
            <v-divider></v-divider>
...
export default {
  ...
  computed: {
    ...mapGetters({
      dataDapil: "dapil/dapilDprdProv",
    }),
  },

  methods: {
    ...mapActions({
      getDataDapil: "dapil/getDapilDprdProv",
      ...
      if (this.state == "SAVE") {
        if (this.foto.length > 0) {
          ...
          this.foto.forEach((item, index) => {
            ...
            }-CAMAT-${index + 1}-${timeStamp}-DPRDPROV.${item.file.name
              .split(".")
              .pop()}`;
            newFoto.push(
